  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-base md:text-lg text-gray-600">{t('teachers.loading')}</div>
      </div>
    );
  }
      ...
      {teachers.length === 0 ? (
        <Card>
          <CardContent className="p-6 md:p-8 text-center">
            <p className="text-sm md:text-base text-gray-600">{t('teachers.noTeachers')}</p>
          </CardContent>
        </Card>
      ) : (
            ...
            <Card key={teacher._id} className="hover:shadow-lg transition-shadow">
              <CardHeader>
                <div className="flex flex-col items-center mb-4">
                  <div className="w-24 h-24 sm:w-32 sm:h-32 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center text-white text-3xl sm:text-4xl font-bold mb-4 overflow-hidden">
                    {teacher.imageUrl ? (
                      <img
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {teacher.specialties.map((specialty, index) => (
                        <span
                          key={index}
                          className="bg-primary-100 text-primary-700 px-2 md:px-3 py-1 rounded-full text-xs font-medium"
                        >
                          {specialty}
                        </span>
                      ))}
                ...
                {teacher.userId.phone && (
                  <div className="mt-4 pt-4 border-t border-gray-200">
                    <a
                      ...
                      className="flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg transition-colors text-xs md:text-sm"
                    >
